 ...
 */
export class PkAuthentication implements SecurityAuthentication {
  /**
...
export function configureAuthMethods(
  ...
  if (config.pk) {
    authMethods.pk = new PkAuthentication(config.pk.tokenProvider)
  }

  if (config.sk) {
    authMethods.sk = new SkAuthentication(config.sk.tokenProvider)
  }
