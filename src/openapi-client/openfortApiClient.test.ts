...
beforeAll(async () => {
  server = createServer((req, res) => {
    ...
    const scripted = responses[req.url ?? ''] ?? {
      status: 200,
      body: { ok: true },
    }
...
describe('request id correlation', () => {
  ...
  it('failed requests carry the id as APIError.correlationId and in the message', async () => {
    ...
    const error = await openfortApiClient({
      method: 'GET',
      url: '/missing',
    }).then(
      () => {
  ...
  it('onRequest fires for successes and failures with matching ids', async () => {
    ...
    await openfortApiClient({ method: 'GET', url: '/bad' }).catch(
      () => undefined,
    )
  ...
  it('a throwing onRequest callback never fails the request', async () => {
    ...
    await expect(
      openfortApiClient({ method: 'GET', url: '/ok' }),
    ).resolves.toEqual({
      ok: true,
    })
  })
