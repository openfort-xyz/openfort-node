...
export class IamApiWrapper extends BaseApiWrapper<AdminAuthenticationApi> {
     ...
     * @param token Specifies the auth token.
     */
    public async verifyOAuthToken(request: AuthenticateOAuthRequest): Promise<PlayerResponse> {
        return await this.api.verifyOAuthToken(request.provider, request);
    }
     ...
     */
    public async createOAuthConfig(body: OAuthConfig): Promise<OAuthConfig> {
        return await this.api.createOAuthConfig(body);
    }
    ...
    public async getAuthPlayers(
        ...
        return await this.api.getAuthPlayers(limit, skip, order, email);
    }
